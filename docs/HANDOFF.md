...
- 首页底栏、详情页互动底栏、下载 PDF 底栏已统一替换为 `BottomBar` 实例，再通过内部 `Type` 切换实际底栏。
...
底栏组件已从一个大 `tabbar` 组件集拆开，并新增 `BottomBar` 作为统一入口。

- `BottomBar` 是外层壳组件：
  - 右侧面板通过 `Type` 实例替换快速切换底栏类型
  - 可切换到 `TabBar`、`DetailActionBar`、`DownloadPDFBar`
  - 内部 `Bar` 实例已设置为 exposed instance
- 切到 `TabBar` 时，暴露的是首页底栏自己的状态：
  - `Active=Brief`
  - `Active=Mine`
- 切到 `DetailActionBar` 时，才暴露互动栏状态：
  - `Liked=True/False`
  - `Collected=True/False`

- 底层组件保持独立：
  - `TabBar` 组件集只保留首页底部切换
  - `DetailActionBar` 独立组件集用于互动栏
  - `DownloadPDFBar` 是独立组件
...
原因：Figma 变体属性是组件集级别的；如果把普通 TabBar、互动栏、下载栏放在同一个组件集里，`Liked` 和 `Collected` 会出现在所有类型右侧面板中。`BottomBar` 用实例替换解决“快速切换类型”，各子组件继续负责自己的状态。
